...
def recommend_movies(movie_title, movie_content_df, cosine_sim, top_n=11):
    ...
    recommendations = movie_content_df.iloc[[i[0] for i in top_movies]][["movie_id", "title", "genre_ids", "rating_average"]]
...
def final_movie_recs(title):
  top_100_movies = recommend_movies(title, movie_content_df, cosine_sim, top_n=101)
...
movie_row = movies_df[movies_df["title"] == title]
if not movie_row.empty:
    movie_row["movie_id"] = movie_row["id"]
    top_100_movies = pd.concat([top_100_movies, movie_row], ignore_index=True)
...
merged_movies_subset = top_100_movies.merge(movies_df, left_on="movie_id", right_on="id", how='left') # just 100 movies
movies_merged = pd.merge(merged_movies_subset, movie_reviews_df, left_on='id', right_on='movie_id')
...
final_recommendations = item_based_recommender(title, movies_pivot=movies_pivot, num_recs=10)
